"""The per-dataset CellAnnotation projection: a read-only snapshot of state.db's
...
With this projection, no worker node ever opens state.db. The only processes that
...
load-bearing:
  ...
  - Carries the portal fetch timestamp and a digest of the cluster set it was
    built for, and read_projection() ENFORCES both. A leftover or foreign file
...
cell_metadata.get_metadata_for). This is a projection of that, never a substitute.
"""
...
PROJECTION_DIR_NAME = "igvf_metadata"
...
# projection is a faithful snapshot rather than a lossy subset.
COLUMNS = [
    "dataset",
    "cluster",
    "cell_annotation",
...
def projection_path(output_dir, dataset):
    return os.path.join(output_dir, PROJECTION_DIR_NAME, f"{dataset}_cell_annotations.tsv")
...
def cluster_set_digest(cluster_keys):
    ...
    projection built for a different cluster set can never be silently accepted --
...
def write_projection(path, rows, fetched_at, digest):
...
def read_projection(path, expected_digest=None, max_age_hours=DEFAULT_MAX_AGE_HOURS, fix_hint=""):
    """{(dataset, cluster): row} for a projection that passes every check.

    Raises ProjectionError -- never returns an empty dict as a stand-in for
    ...
    if not os.path.exists(path):
        raise ProjectionError(f"no CellAnnotation projection at {path}{hint}")
    ...
    if expected_digest is not None and provenance.get("cluster_set_digest") != expected_digest:
        raise ProjectionError(
            ...
            f"(projection {provenance.get('cluster_set_digest')!r} != config {expected_digest!r}) -- "
    ...
    if max_age_hours is not None:
        if not fetched_at:
            raise ProjectionError(f"{path} records no portal_fetched_at, so its age can't be checked{hint}")
