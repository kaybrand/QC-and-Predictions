...
## Forked from scE2G/workflow/scripts/prediction_qc/plot_all_qc_stats.R.
##
## The original always rebuilds all_qc_stats.tsv from scratch by concatenating
## a `stats_files` list scoped to whatever clusters are in scE2G's own
## BIOSAMPLE_DF (i.e. only clusters in THIS pipeline's own cell_clusters.tsv
## for the current run) -- it can't preserve rows for clusters processed
## outside this pipeline, or clusters simply untouched in a given invocation.
##
## Here, aggregate_qc_stats.py has already built the complete, dataset-wide
## all_qc_stats.tsv (every cluster directory on disk, not just this run's),
## so this script skips the concatenation step entirely and just reads that
## file directly -- every plotting function below is otherwise unchanged.
...
## Read the already-complete, dataset-wide all_qc_stats.tsv (built by aggregate_qc_stats.py)
stats <- fread(snakemake@input$all_stats) %>% as_tibble()
